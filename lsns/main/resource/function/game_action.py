...
def startupapp(appname="com.nineyou.fuxiao"):
    start_app(appname)
    # 循环检测启程图标，检测到就结束，没有就疯狂点击左下角（(*^_^*)
    flag = True
    while flag:
        loc = exists(Template(filename="resource/template/guide/shop_ui.png", resolution=(1280, 720)))
        if loc:
            return True
        loc = exists(Template(filename="resource/template/guide/daily_sign.png", resolution=(1280, 720)))
        if loc:
            touch(v=(1240, 105))
        touch(v=(1130, 70))


def closeapp(appname="com.nineyou.fuxiao"):
    stop_app(appname)
...
def rotate_oil_product(character_choose_list=None):
    if character_choose_list:
        character_choose_list = []
    guide.enter_port()
    while True:
        if loc := base.find_text_include_ocr(text="炼油厂"):
            base.touch(loc)
            base.sleep(1)
            break
    while True:
        if loc := base.find_text_include_ocr(text="交换"):
            base.touch(loc)
            base.sleep(1)
            break
    while True:
        if loc := base.find_text_include_ocr(text="筛选"):
            base.touch(loc)
            base.sleep(1)
            break
    while True:
        if loc := base.find_text_include_ocr(text="星数"):
            base.touch(loc)
            base.sleep(1)
            break
    base.touch((1120, 40))
    ...
    offset_y = 285
    for i in range(14):
        base.touch((base_loc[0] + i % 7 * offset_x, base_loc[1] + i // 7 * offset_y))
        if loc := base.find_text_include_ocr(text="取消", rect=[530, 430, 450, 65]):
            base.touch(loc)
            base.sleep(1)
        if int(base.recognition_text_ocr([575, 35, 45, 22]).split("/")[0]) >= 3:
            break
    base.touch((1200, 660))
    guide.backmain()


def rotate_shop_product(character_choose_list=None):
    if character_choose_list:
        character_choose_list = []
    guide.enter_port()
    while True:
        if loc := base.find_text_include_ocr(text="便利店"):
            base.touch(loc)
            base.sleep(1)
            break
    while True:
...
def rotate_rest_product(character_choose_list=None):
    # todo 这个是在温泉休息的函数
    if character_choose_list:
        character_choose_list = []
    guide.enter_port()
    base.swipe(start=(1000, 360), end=(200, 360))
    while True:
        if loc := base.find_text_include_ocr(text="温泉"):
            base.touch(loc)
            base.sleep(1)
            break
    while True:
...
def expedition_action():
    guide.enter_battle()
    guide.enter_fight_type("远征")
...
def clean_trade_mission(loc_city, human_transport, freight_transport, purchase_transport):
    ...
    print("执行路径:", "->".join([base.city_name_transition(i) for i in plan]), "消耗疲劳", cost)

    for i in plan[1:]:
        print("购买临时商品填补空缺")
        count.update_product_inf()
        temp_buy_list = count.t4(start_city=loc_city, end_city=i)
