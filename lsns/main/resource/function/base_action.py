...
def find_text_exist_ocr(textlist=None, rect=None):
    if not textlist:
        print("空参数")
        return False
    print("搜索文本")
    ...
    for idx in range(len(result)):
        ...
        for line in res:
            if line[1][0] in textlist:
                loc = [(line[0][0][0] + line[0][1][0]) // 2, (line[0][1][1] + line[0][2][1]) // 2]
                return line[1][0], [loc[0] + rect[0], loc[1] + rect[1]]
    return False


def find_textlist_ocr(textlist=None, rect=None):
    if not textlist:
        print("空参数")
        return False
    print("搜索文本")
...
def city_name_transition(name=""):
    ...
    citylist = [["阿妮塔能源研究所", "anita_energy_research_institute"],
                ["7号自由港", "freeport"],["七号自由港", "freeport"],["7号自电港", "freeport"],["7号自电港3", "freeport"],
                ["澄明数据中心", "clarity_data_center_administration_bureau"],
                ["修格里城", "shoggolith_city"],["修格果城", "shoggolith_city"],["修格男城", "shoggolith_city"],
                ["铁盟哨站", "brcl_outpost"],
...
def recognition_text_ocr(rect=None)->str:
    if rect is None:
        rect = [0, 0, 1280, 720]
    screen = G.DEVICE.snapshot()
...
def recognition_color_ocr(color=None, distance=150, rect=None):
    if not color:
        print("无颜色参数")
        raise TypeError("no color")
    if rect is None:
        rect = [0, 0, 1280, 720]
        # print("空参数运行")
        # raise TypeError("no rect")
    screen = G.DEVICE.snapshot()
    result = []
    print("识别颜色范围", rect)
    newscreen = screen[rect[1]:rect[1] + rect[3], rect[0]:rect[0] + rect[2]]
    avgrgb = [np.mean([[i[2] for i in lin1] for lin1 in newscreen], dtype=int),
              np.mean([[i[1] for i in lin1] for lin1 in newscreen], dtype=int),
              np.mean([[i[0] for i in lin1] for lin1 in newscreen], dtype=int)]
    # 这里解出来是bgr顺序，已经改成rgb顺序
    compare = math.sqrt(sum([(i[0] - i[1]) ** 2 for i in zip(color, avgrgb)]))
    print("比对颜色", color, "和", avgrgb, "差值为", compare)
    if compare <= distance:
        return True
    return False


def count_nearest_city(city=None, citylist=None):
    """
    这里是计算最近城市的，估计要删
    """
    if city is None:
    ...
    for i in citylist:
        if i == city:
            continue
        print(city_name_transition(city), "→", city_name_transition(i),
              int(temp[all_city_list.index(city)][all_city_list.index(i)]))
...
def route_planning(start_city=None, city_list: list = None):
    ...
    if not city_list:
        travel_list = all_city_list
    else:
        travel_list: list = city_list.copy()

    if start_city not in travel_list:
        travel_list.insert(0, start_city)
    else:
        travel_list.remove(start_city)
        travel_list.insert(0, start_city)

    cost_matrix = np.zeros((len(travel_list), len(travel_list)), int)

    for i in range(len(travel_list)):
        for j in range(len(travel_list)):
            cost_matrix[i][j] = city_tired_list[
                all_city_list.index(travel_list[i]), all_city_list.index(travel_list[j])]
    start_city = 0
    print(cost_matrix)
    shortest_path, total_cost = tsp_dp(cost_matrix, start_city)

    result_path = [travel_list[i] for i in shortest_path]

    print("选取路径:", "->".join(result_path))
    print("消耗疲劳:", total_cost)
    return total_cost, result_path
...
def recognition_by_model_yolo(model_name:str="resource/model/monster_lostgoods.pt",img=None):
    # todo 推理速度不够快，还得改
    pass
    # # Load a pretrained YOLOv8n model
    # model = YOLO(r"resource/model/monster_lostgoods.pt")
    #
    #
    # screen = img if img else G.DEVICE.snapshot()
    #
    # result = model.predict(screen, imgsz=640,conf = 0.6, device="cpu")
    #
    # return_list = []
    # for i in result:
    #     for j in i.boxes.xywh:
    #         x = j[0].item()  # x坐标为列表中的第一个
    #         y = j[1].item()  # y坐标为列表中的第二个
    #         print("Y：识别到目标",int(i.boxes.cls.item()),"坐标",(int(x),int(y)))
    #         return_list.append([int(i.boxes.cls.item()),(int(x),int(y))])
    #
    # return return_list