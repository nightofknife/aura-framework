...
@service_info(alias="screen", public=True, deps={"config": "core/config"})
class ScreenService:
    """
    ...
    def __init__(self, config: ConfigService):
        ...
        IMPORTANT: Config values are accessed dynamically via properties
        to support plan-specific configuration.
        """
        self.config = config
        self.hwnd = None
        self._update_hwnd()
    ...
    # =========================================================================
    # Configuration properties (dynamic access via ConfigService)
    # =========================================================================

    @property
    def enabled_backends(self) -> list[str]:
        """Get enabled backends from current plan's config."""
    ...
    def default_backend(self) -> str:
        """Get default backend from current plan's config."""
        # Check runtime preference first
        if hasattr(self, '_runtime_default_backend') and self._runtime_default_backend:
            if self._runtime_default_backend in self.enabled_backends:
    ...
    @property
    def max_black_ratio(self) -> float:
        """Get max black ratio threshold from config."""
        return float(self.config.get('screen.capture.max_black_ratio', 0.98))

    @property
    def min_stddev(self) -> float:
        """Get min standard deviation threshold from config."""
        return float(self.config.get('screen.capture.min_stddev', 5.0))

    @property
    def min_edge_ratio(self) -> float:
        """Get min edge ratio threshold from config."""
        return float(self.config.get('screen.capture.min_edge_ratio', 0.001))
    ...
    def target_title(self) -> Optional[str]:
        """Dynamic get current execution context's target window title."""
        title = self.config.get('app.target_window_title', None)
        return title
    ...
    def set_default_backend(self, backend: str):
        """Set runtime backend preference (not persisted)."""
    ...
    def _capture_with_fallback_sync(self, rect: tuple[int, int, int, int] | None,
        ...
        for name in backends:
            ...
            last_result = result
            if result.success:
                self._runtime_default_backend = name  # ✅ Set runtime preference
                return result
    ...
    def _capture_backend_sync(self, backend: str, context: Dict[str, Any]) -> CaptureResult:
        backend = (backend or "").lower()
        if backend not in self.enabled_backends:  # ✅ Use property
    ...
    def _evaluate_image_quality(self, image: np.ndarray,
        ...
        if black_ratio >= self.max_black_ratio:  # ✅ Use property
            flags.append("black_frame")

        stddev = float(gray.std())
        if stddev < self.min_stddev:  # ✅ Use property
            flags.append("low_variance")
            edges = cv2.Canny(gray, 50, 150)
            edge_ratio = float(np.mean(edges > 0))
            if edge_ratio < self.min_edge_ratio:  # ✅ Use property
    ...
    def _get_backend_order(self) -> List[str]:
        enabled = list(self.enabled_backends)  # ✅ Use property
        default = self.default_backend  # ✅ Use property
    ...
    def _get_primary_monitor_rect(self) -> tuple[int, int, int, int]:
        try:
            ...
            raise RuntimeError(
                "ScreenService requires optional dependency 'screeninfo'. "
                "Install it with `pip install -r plans/aura_base/requirements.txt`."
            ) from exc
