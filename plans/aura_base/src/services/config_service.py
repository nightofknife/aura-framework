...
from packages.aura_core.context.plan import current_plan_name  # 从核心模块导入

# 为了向后兼容，重新导出 current_plan_name
__all__ = ['ConfigService', 'current_plan_name']


@register_service(alias="config", public=True)
class ConfigService:
    """Configuration service - facade over PlanContext.config layer.

    【分层配置 v4.0 - 上下文隔离 + PlanContext集成】

    This service provides dynamic, context-aware configuration access:
    - Uses ContextVar (current_plan_name) to identify the active plan
    - Each plan's config is isolated in PlanContext.config (read-only)
    - Merges: env vars > global config.yaml > plan config.yaml

    Integration with PlanContext:
    - PlanContext is created by Orchestrator when a plan is loaded
    - PlanContext.config layer contains the plan's config.yaml data
    - This service dynamically builds the config chain on each get() call
    - Services should NOT cache config in __init__, use dynamic properties instead

    Configuration hierarchy:
    1. Environment variables (AURA_* prefix, highest priority)
    2. Global config.yaml (project root)
    3. Plan config.yaml (plans/{plan_name}/config.yaml, lowest priority)

    Usage for Service Developers:
        # ✅ CORRECT: Use dynamic properties
        class MyService:
            def __init__(self, config: ConfigService):
                self.config = config

            @property
            def my_value(self) -> Optional[str]:
                return self.config.get('my.setting')

        # ❌ WRONG: Caching in __init__
        class MyService:
            def __init__(self, config: ConfigService):
                self.my_value = config.get('my.setting')  # Don't cache!
    """
