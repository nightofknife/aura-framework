...
class TaskLoader:
    ...
    ✅ 修复：添加全局缓存失效机制和文件级锁，确保线程安全和缓存一致性。
    """
    ...
    def invalidate_all_caches(cls):
        ...
        ✅ 新增：使所有TaskLoader实例的缓存失效。
        用于热重载或配置更新时。
        """
        with cls._version_lock:
            cls._global_cache_version += 1
            logger.info(f"全局任务缓存已失效，版本: {cls._global_cache_version}")
    ...
    def _load_and_parse_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        ✅ 修复：添加文件级锁，防止并发加载同一文件。
        """
        # ✅ 检查缓存是否有效
        if not self._is_cache_valid():
            logger.debug(f"缓存版本过期，清空缓存 (plan: {self.plan_name})")
            self.cache.clear()
        ...
        with TaskLoader._locks_lock:
            ...
            file_lock = TaskLoader._file_locks[file_path_str]
