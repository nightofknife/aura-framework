...
class RunQueryService:
    ...
    def get_all_services_status(self) -> List[Dict[str, Any]]:
        ...
        if self._scheduler._loop and self._scheduler._loop.is_running():
            future = asyncio.run_coroutine_threadsafe(async_get_services(), self._scheduler._loop)
            try:
                return future.result(timeout=2)
            except Exception as exc:
                logger.error("Failed to get services status asynchronously: %s", exc)
    ...
    def get_all_interrupts_status(self) -> List[Dict[str, Any]]:
        ...
        if self._scheduler._loop and self._scheduler._loop.is_running():
            future = asyncio.run_coroutine_threadsafe(async_get(), self._scheduler._loop)
            try:
                return future.result(timeout=2)
            except Exception as exc:
                logger.error("Failed to get interrupts status asynchronously: %s", exc)
