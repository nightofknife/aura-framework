...
class InterruptService:
    ...
    def __init__(self, scheduler):
        ...
        self.is_running = None  # 将在 run() 中初始化
        ...
        self._stop_requested = None  # 将在 run() 中初始化

    async def run(self):
        """服务的主循环，负责持续监控中断条件，支持优雅停止。

        此方法会循环运行，直到收到停止信号。在每个循环中，它会：
        1. 获取当前所有应被激活的中断规则列表。
        ...
        self.is_running.set()
        try:
            while not self._stop_requested.is_set():  # ✅ 改用停止标志
                ...
                try:
                    ...
                    logger.info("收到停止信号，退出中断监控循环")
                    break
                except asyncio.TimeoutError:
                    # 正常超时，继续循环
                    pass
        except asyncio.CancelledError:
            logger.info("中断监控服务 (InterruptService/Guardian) 被取消")
            raise
        finally:
    ...
    def stop(self):
        """请求停止服务"""
        logger.info("请求停止 InterruptService...")
