...
class SchedulingService:
    ...
    async def run(self):
        ...
        此方法会循环运行，直到收到停止信号。为了提高调度的准确性，
        它会计算并等待到下一个tick时刻再执行检查。
        """
        ...
        try:
            ...
            logger.info("时间基准调度服务被取消")
            raise
        finally:
            self.is_running.clear()
            logger.info("时间基准调度服务已停止")

    def stop(self):
        """请求停止服务"""
        logger.info("请求停止 SchedulingService...")
